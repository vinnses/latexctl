...
setup_sandbox() {
  ...
  cp -R "${REPO_ROOT}" "$sandbox/repo/latexctl"
  rm -rf "$sandbox/repo/latexctl/.git"
  rm -f "$sandbox/repo/latexctl/package-overrides.conf"
  cp -R "${REPO_ROOT}/tests/fixtures/${fixture}/tex/." "$sandbox/repo/tex/"

  cat > "$sandbox/repo/Makefile" <<'EOF'
.RECIPEPREFIX := >

MAIN_TEX ?= tex/main.tex
LATEXMK_ARGS = -cd -pdf -interaction=nonstopmode -file-line-error

.PHONY: build clean pdf watch

build:
> latexctl/bin/latexctl build $(MAIN_TEX)

clean:
> find tex -type f \( -name '*.aux' -o -name '*.log' -o -name '*.out' -o -name '*.toc' -o -name '*.bbl' -o -name '*.bcf' -o -name '*.blg' -o -name '*.fdb_latexmk' -o -name '*.fls' -o -name '*.run.xml' -o -name '*.synctex.gz' \) -delete

pdf:
> TEXINPUTS="$$PWD/tex//:$${TEXINPUTS:-}" BIBINPUTS="$$PWD/tex//:$${BIBINPUTS:-}" BSTINPUTS="$$PWD/tex//:$${BSTINPUTS:-}" latexmk $(LATEXMK_ARGS) $(MAIN_TEX)
> latexctl/bin/latexctl report-build --status success --log tex/main.log --output build-report.md

watch:
> TEXINPUTS="$$PWD/tex//:$${TEXINPUTS:-}" BIBINPUTS="$$PWD/tex//:$${BIBINPUTS:-}" BSTINPUTS="$$PWD/tex//:$${BSTINPUTS:-}" latexmk -cd -pdf -pvc -interaction=nonstopmode -file-line-error $(MAIN_TEX)
EOF
