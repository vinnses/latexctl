...
latexctl_repo_root() {
  local tooling_root
  local tooling_parent

  if [[ -n "${LATEXCTL_PROJECT_ROOT:-}" ]]; then
    cd "${LATEXCTL_PROJECT_ROOT}" && pwd
    return 0
  fi

  tooling_root="$(latexctl_tooling_root)"
  tooling_parent="$(dirname "${tooling_root}")"

  if [[ "$(basename "${tooling_root}")" == "latexctl" && -d "${tooling_parent}/tex" ]]; then
    cd "${tooling_parent}" && pwd
    return 0
  fi
